...
class COREPLUGINSHARED_EXPORT IResourceFactory : public QObject {
	...
	/*!
		...
		\ingroup Resources
		\brief This class manages the life cycle of resources and is the interface between the QuickHub resource system
	   and the resources provided by your plugin. This class must be subclassed and implemented for each resource your
	   plugin provides. An instance of this class must be registered at the ResourceManager. Everytime a resource is
	   requested by a client, the resource manager will look for factories that are able to provide an instance of the
	   appropriate resource.

		The developer of a resource can decide whether the instance of a resource is kept or released after use.
		Resources are always managed internally with SmartPointern. Accordingly, the release is done automatically as
	   soon as the last client has detached itself from the resource. Alternatively, the factory can keep its own
	   SmartPointer and always return it instead of creating a new instance each time.

		\sa ResourceManager
	*/
public:
	// ctor
	IResourceFactory(QObject* parent = nullptr)
		: QObject(parent)
	{
	}
	// dtor
	...
	/*!
		...
		Normally this function does not need to be overwritten.  This function ensures that the Device Manager can
	   distinguish between resources that differ despite having the same descriptors

		For example, "home/myList" will contain different data depending on who wants to access the resource.
		In order for the ResourceManager to be able to distinguish between these resources, a unique string (some kind
	   of uuid) must be generated from the descriptor. (this is what generateQualifiedResourceName will do in the
	   default implementation)

		You can overwrite the behavior to make sure that different resources for the same descriptor can be
	   distinguished.
	*/
	...
	*/
	...
	/*!
		...
		to provide an instance of this resource.  Assuming its resource provides access to user objects. Then a
	   reasonable prefix would be "myplugin/users".

		A request for the resource "myplugin/users/#4711" would result in the corresponding resource factory being
	   called. In consequence, the createResource() function would be called with the appropriate descriptor.
	*/
	virtual QString getDescriptorPrefix() const { return ""; }

private:
	/*!
		\fn resourcePtr IResourceFactory::createResource( QString descriptor, QString token ="", QObject* parent =
	   nullptr) This is the most important function. It is responsible for returning an instance to the appropriate
	   resource. It's okay to return a nullpointer when the parameters are invalid or the resource can't be found.
	*/
